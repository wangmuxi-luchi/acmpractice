...
XC以及05级的COY组成。在共同的集训生活中，大家建立了深厚的友谊，阿牛准备做点什么来纪念这段激情燃烧的岁月，
...
int main()
{
	...
	for(int i=1;i<40;i++)
	{
		...
		step[1][i]=step[0][i-1]+step[1][i-1]+step[2][i-1];
		step[2][i]=step[0][i-1]+step[1][i-1]+step[2][i-1];
	}
	int n;
	while(cin>>n)
	{
		cout<<step[0][n-1]+step[1][n-1]+step[2][n-1]<<endl;
	}
}
